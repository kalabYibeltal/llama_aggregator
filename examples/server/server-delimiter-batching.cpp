...
// std::shared_ptr<std::vector< std::vector<json>, int> > prompt_buffer = std::make_shared<std::vector< std::vector<json>, int> >();
// std::reference_wrapper<httplib::Response>
using PromptBufferElement = std::tuple<json, std::reference_wrapper<httplib::Response> , int>;
using PromptBufferElementVectorized = std::tuple<json, std::vector<std::reference_wrapper<httplib::Response>> , std::vector<int>>;
std::shared_ptr<std::vector< PromptBufferElement >> prompt_buffer = std::make_shared<std::vector< PromptBufferElement >>();

auto response_deleter = [](httplib::Response* response) {
    // Perform any necessary cleanup or modifications here
    delete response;
};

// std::shared_ptr<std::vector<json>> output_buffer = std::make_shared<std::vector<json>>();
std::atomic<int> request_id(0);

// using ElementType = std::tuple<bool, std::string, auto, auto>;

// using ElementType = std::tuple<bool, std::string, std::function<bool(size_t offset, httplib::DataSink &sink)>, auto>;

// Create a shared pointer to a map
// std::shared_ptr<std::map<int, std::vector<ElementType>>> output_buffer =  std::make_shared<map<int, std::vector<ElementType>>>();

std::shared_ptr<std::map<int, bool>> output_buffer =  std::make_shared<std::map<int, bool>>();
...
// std::vector<json> prompt_buffer,
// bool running2,

void process_batch_prompts (
            const std::function<void(server_task_inf_type, json&, std::vector<std::reference_wrapper<httplib::Response>>&, std::vector<int>& )> handle_completions_generic,
        ...
        while (true) {
            ...
            int i = 0;
            while (prompt_buffer->size() < 14 && (i < 5 || prompt_buffer->empty())) {
            ...
            std::vector< PromptBufferElement > prompts_to_process;
                ...
                if (!prompt_buffer->empty()) {
                    ...
                    prompts_to_process = std::move(*prompt_buffer);  // Move prompts out of buffer
                    prompt_buffer->clear();  // Clear buffer after moving
                    lock.unlock();
                }
            // }
            
            
            std::vector< PromptBufferElementVectorized > prompts_to_process_vectorized;
            std::string batched_input = "Each question below is separated by ###. Answer each question briefly and return the answers seperated by ###.";
            std::vector<int> batched_ids;
            std::vector<std::reference_wrapper<httplib::Response>> batched_responses;  // Array of Response objects

            // //////////// batching 
            if (!prompts_to_process.empty()) {
                // Concatenate all prompts with a delimiter
                for (size_t k = 0; k < prompts_to_process.size(); ++k) {
                    if (k > 0) {
                        batched_input += "\n###\n"; // Add delimiter
                    }
                    batched_input += (std::get<0>(prompts_to_process[k])).at("prompt"); // Concatenate prompt JSON data
                    batched_responses.push_back(std::get<1>(prompts_to_process[k])); 
                    batched_ids.push_back(std::get<2>(prompts_to_process[k])); 
                }
            }
         


            std::cout << "Batched Input: " << batched_input << std::endl;

            // // // // Process the batch of prompts if there are any 
            json batched_json = json{{"prompt", batched_input}};
            handle_completions_generic(SERVER_TASK_INF_TYPE_COMPLETION, batched_json, batched_responses, batched_ids);
...
int main(int argc, char ** argv) {
    ...
    const auto handle_completions_generic = [&ctx_server, &res_error, &res_ok, &output_buffer_mutex,&buffer_mutex ](server_task_inf_type inf_type, json &data, std::vector<std::reference_wrapper<httplib::Response>> &res, std::vector<int> &batched_ids) {
        ...
        if (!stream) {
            ...
            ctx_server.receive_cmpl_results(task_ids, [&](std::vector<server_task_result> & results) {
                ...
                if (results.size() == 1) {
                    ...
                    std::cout << "Results Data: " << results[0].data.dump() << std::endl;
                    std::string batched_result = results[0].data.at("content"); // Assuming batched results are in results[0].data
                    std::cout << "Batched result: " << batched_result << std::endl;
                    const std::string& delimiter = "###";
                    std::vector<std::string> split_results;

                    size_t start = 0, end = 0;
                    while ((end = batched_result.find(delimiter, start)) != std::string::npos) {
                        split_results.push_back(batched_result.substr(start, end - start));
                        start = end + delimiter.length();
                    }
                    split_results.push_back(batched_result.substr(start)); // Add the final segment


                    // Ensure the number of split results matches the number of responses
                    if (split_results.size() != res.size()) {
                        // Handle mismatch (log error or throw exception)
                        throw std::runtime_error("Mismatch between batched results and response objects!");
                    }

                    for (size_t i = 0; i < res.size(); ++i) {

                        if (i < split_results.size()) {
                            res_ok(res[i].get(), split_results[i]); // Assign result to response object
                        }
                        else {
                            res_ok(res[i].get(), "**EMPTY**");
                        }

                        std::unique_lock<std::mutex> lock(output_buffer_mutex);  
                        int id = batched_ids[i];
                        (*output_buffer)[id] = true; // Mark the corresponding ID as processed
                        lock.unlock();
                    }
                    ...
                    // std::unique_lock<std::mutex> lock(output_buffer_mutex);
                    // for (size_t i = 0; i < batched_ids.size(); ++i) { 
                    //     int id = batched_ids[i];
                    //     (*output_buffer)[id] = true; // Set the corresponding ID in the output buffer to true
                    // }
                    // lock.unlock();
                }
                // } else {
    ...
    const auto handle_completions = [&handle_completions_generic, &buffer_mutex, &output_buffer_mutex](const httplib::Request & req, httplib::Response & res) {
        
        auto start = std::chrono::high_resolution_clock::now();

        json data = json::parse(req.body);

        // put res in prompt buffer 
        ++request_id;
        ...
        // monitor the output buffer for id , if the id is present we will return 

        

        while(true) {
            std::unique_lock<std::mutex> lock3(output_buffer_mutex);
            if ( (*output_buffer)[id] ){
                ...
                // Add the counter field to the JSON object
        ...
        // res.set_content("Request received and will be processed in batch.", "text/plain");
    ...
    const auto handle_infill = [&ctx_server, &res_error, &handle_completions_generic](const httplib::Request & req, httplib::Response & res) {
        ...
        // return handle_completions_generic(SERVER_TASK_INF_TYPE_INFILL, data, res,0);
    };
...
// std::ref(prompt_buffer),
    
    // std::thread main_loop_thread( ctx_server.queue_tasks.start_loop );
    
    // main_loop_thread.join();
    // std::thread main_loop_thread([&ctx_server]() {
